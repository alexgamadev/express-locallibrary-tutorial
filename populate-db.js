...
console.log('This script populates some test books, authors, genres and bookcopies to your database. Specified database as argument - e.g.: node populatedb "mongodb+srv://cooluser:coolpassword@cluster0.lz91hw2.mongodb.net/local_library?retryWrites=true&w=majority"');
...
function bookCopyCreate(book, imprint, due_back, status, cb) {
  bookcopydetail = { 
    book: book,
    imprint: imprint
  }    
  if (due_back != false) bookcopydetail.due_back = due_back
  if (status != false) bookcopydetail.status = status
    
  const bookcopy = new BookCopy(bookcopydetail);    
  bookcopy.save(function (err) {
    if (err) {
      console.log('ERROR CREATING BookCopy: ' + bookcopy);
      cb(err, null)
      return
    }
    console.log('New BookCopy: ' + bookcopy);
...
function createBookCopies(cb) {
    async.parallel([
        function(callback) {
          bookCopyCreate(books[0], 'London Gollancz, 2014.', false, 'Available', callback)
        },
        function(callback) {
          bookCopyCreate(books[1], ' Gollancz, 2011.', false, 'Loaned', callback)
        },
        function(callback) {
          bookCopyCreate(books[2], ' Gollancz, 2015.', false, false, callback)
        },
        function(callback) {
          bookCopyCreate(books[3], 'New York Tom Doherty Associates, 2016.', false, 'Available', callback)
        },
        function(callback) {
          bookCopyCreate(books[3], 'New York Tom Doherty Associates, 2016.', false, 'Available', callback)
        },
        ...
        function(callback) {
          bookCopyCreate(books[4], 'New York, NY Tom Doherty Associates, LLC, 2015.', false, 'Available', callback)
        },
        function(callback) {
          bookCopyCreate(books[4], 'New York, NY Tom Doherty Associates, LLC, 2015.', false, 'Maintenance', callback)
        },
        function(callback) {
          bookCopyCreate(books[4], 'New York, NY Tom Doherty Associates, LLC, 2015.', false, 'Loaned', callback)
        },
        function(callback) {
          bookCopyCreate(books[0], 'Imprint XXX2', false, false, callback)
        },
        function(callback) {
          bookCopyCreate(books[1], 'Imprint XXX3', false, false, callback)
        }
        ],
        // Optional callback
        cb);
